...
export async function validateEventRoute(app: FastifyInstance): Promise<void> {
  app.post("/validate/event", {
    ...
    schema: {
      ...
      description: "Valida um único evento eSocial antes do envio ao governo",
    },
  }, async (request, reply) => {
    ...
    // Incrementa quota e persiste log — assíncrono, não bloqueia resposta
    Promise.all([
      TenantRepository.incrementQuota(tenantId, 1),
      getDb()`
        INSERT INTO validation_logs
          (tenant_id, event_id, status, score, total_rules, passed_rules, issue_count, duration_ms)
        VALUES (
          ${tenantId}, ${result.eventId}, ${result.status}, ${result.score},
