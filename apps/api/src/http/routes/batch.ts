...
export async function validateBatchRoute(app: FastifyInstance): Promise<void> {
  app.post("/validate/batch", { preHandler: [requireApiKey, requireQuota], schema: { tags: ["Validação"] } },
    async (request, reply) => {
      const body = request.body as { events?: unknown[] };
      if (Array.isArray(body?.events) && body.events.length > BATCH_LIMIT) throw new BatchTooLargeError(BATCH_LIMIT);
