...
export interface RegisterInput {
  tenantName: string; email: string; password: string; plan?: Plan;
}
export interface RegisterResult { tenantId: string; apiKey: string; }
export interface LoginInput { email: string; password: string; }
export interface LoginResult { tenantId: string; accessToken: string; expiresIn: number; }
...
export const AuthService = {
  async register(input: RegisterInput): Promise<RegisterResult> {
    ...
    const tenant = await TenantRepository.create({ name: input.tenantName, plan: input.plan ?? "starter" });
  ...
  async login(input: LoginInput): Promise<LoginResult> {
    const db = getDb();
    const [user] = await db<Array<{ tenantId: string; passwordHash: string; passwordSalt: string }>>`
...
function issueJwt(tenantId: string): { token: string; expiresIn: number } {
  ...
  const sig = createHmac("sha256", config.auth.jwtSecret).update(`${header}.${payload}`).digest("base64url");
