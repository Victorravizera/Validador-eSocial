...
function getDb() {
    ...
    _db = (0, postgres_1.default)(index_js_1.config.db.url, {
        min: index_js_1.config.db.poolMin,
        max: index_js_1.config.db.poolMax,
        idle_timeout: 30,
        connect_timeout: 10,
        ssl: index_js_1.config.server.isProd ? { rejectUnauthorized: true } : false,
        onnotice: function () { },
        debug: index_js_1.config.server.isDev
            ? function (_conn, query) { logger_js_1.logger.debug({ query: query.slice(0, 200) }, "db:query"); }
            : false,
    });
