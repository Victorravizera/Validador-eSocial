...
var INSS_ALIQUOTA_MIN = 0.075;
var INSS_ALIQUOTA_MAX = 0.14;
exports.s1200Rules = [
    {
    ...
    },
    {
        ...
        validate: function (_a) {
            ...
            if (typeof perApur !== "string")
                return null;
            var d = (0, validators_js_1.parsePeriod)(perApur);
    ...
    },
    {
        ...
        severity: "error",
        validate: function (_a) {
            var vrRemun = _a.vrRemun;
            var v = Number(vrRemun);
    ...
    },
    {
        ...
        severity: "warning",
        validate: function (_a) {
            var vrRemun = _a.vrRemun, tpRegTrab = _a.tpRegTrab;
            var v = Number(vrRemun);
            if (isNaN(v))
                return null;
            if (String(tpRegTrab) === "1" && v < (0, validators_js_1.getSalarioMinimo)()) {
                return {
                    field: "vrRemun",
                    code: "S1200_REMUN_SALARIO_MINIMO",
                    message: "Remunera\u00E7\u00E3o (R$ ".concat(v.toFixed(2), ") abaixo do sal\u00E1rio m\u00EDnimo vigente (R$ ").concat((0, validators_js_1.getSalarioMinimo)().toFixed(2), ")"),
                    severity: "warning",
                };
            }
            return null;
    ...
    },
    {
        ...
        severity: "warning",
        validate: function (_a) {
            var vrDescInss = _a.vrDescInss, vrRemun = _a.vrRemun;
            var inss = Number(vrDescInss);
            var remun = Number(vrRemun);
            if (isNaN(inss) || isNaN(remun) || remun === 0)
                return null;
            var aliquota = inss / remun;
            if (aliquota < INSS_ALIQUOTA_MIN || aliquota > INSS_ALIQUOTA_MAX) {
                return {
                    field: "vrDescInss",
                    code: "S1200_INSS_ALIQUOTA",
                    message: "Al\u00EDquota INSS calculada (".concat((aliquota * 100).toFixed(2), "%) fora do range esperado (7,5%\u201314%) \u2014 verifique a tabela progressiva"),
                    severity: "warning",
                };
            }
            return null;
